...
import type { MembroDetalhe } from '@/lib/ministerios';
...
export function AbaPresenca({ membros, encontros, presencas }: AbaPresencaProps) {
  ...
  const idsUltimos6 = new Set(ultimos6.map((e) => e.id));

  // presença por (encontro, usuario)
  const mapa = useMemo(() => {
    const m = new Map<string, MinisterioPresenca>();
    for (const p of presencas) m.set(`${p.encontro_id}:${p.usuario_id}`, p);
    return m;
  }, [presencas]);
  ...
  const frequencia = useMemo(() => {
    ...
    for (const membro of membros) {
      ...
      for (const e of ultimos6) {
        if (mapa.get(`${e.id}:${membro.usuario_id}`)?.presente) presentes += 1;
      }
      freq.set(membro.usuario_id, Math.round((presentes / ultimos6.length) * 100));
    }
    return freq;
  }, [membros, ultimos6, mapa]);
  ...
  const freq3m = useMemo(() => {
    ...
    for (const membro of membros) {
      let presentes = 0;
      for (const e of encontros3m) if (mapa.get(`${e.id}:${membro.usuario_id}`)?.presente) presentes += 1;
      freq.set(membro.usuario_id, Math.round((presentes / encontros3m.length) * 100));
    }
    return freq;
  }, [membros, encontros, mapa]);

  if (encontros.length === 0 || membros.length === 0) {
    return (
      <EmptyState
        icon={CalendarClock}
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="mt-3 space-y-3">
          {membros.map((m) => {
            const f = frequencia.get(m.usuario_id) ?? 0;
            const gargalo = (freq3m.get(m.usuario_id) ?? 100) < 50;
            return (
              <div key={m.usuario_id}>
                ...
                <div className="mt-1 h-2.5 w-full rounded-full bg-bg-page">
                  <div
                    ...
                    style={{ width: `${f}%`, backgroundColor: gargalo ? '#854F0B' : '#3C3489' }}
                  />
                </div>
              </div>
            );
      ...
      <div>
        ...
        <div className="mt-3 overflow-x-auto rounded-lg border border-border">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {membros.map((m) => (
                <tr key={m.usuario_id} className="border-b border-border last:border-b-0">
                  <td className="px-3 py-2 text-text-primary">{m.nome}</td>
                  {ultimos6.map((e) => {
                    const p = mapa.get(`${e.id}:${m.usuario_id}`);
                    let simbolo = '—';
