...
import { criarEncontroComPresencas, type MembroDetalhe } from '@/lib/ministerios';
...
export function RegistrarEncontroModal({
  ...
  useEffect(() => {
    if (open) {
      ...
      membros.forEach((m) => (inicial[m.usuario_id] = { presente: true, justificativa: '' }));
    ...
    try {
      await criarEncontroComPresencas(
        ...
        membros.map((m) => ({
          ...
          presente: presencas[m.usuario_id]?.presente ?? false,
          justificativa: presencas[m.usuario_id]?.justificativa || undefined,
        }))
      );
      onRegistrado();
      onClose();
      ...
      <form onSubmit={handleSalvar} className="space-y-3">
        ...
        <div>
          ...
          <div className="max-h-64 space-y-2 overflow-y-auto">
            {membros.map((m) => {
              const estado = presencas[m.usuario_id] ?? { presente: true, justificativa: '' };
              return (
                <div key={m.usuario_id} className="rounded-md border border-border p-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-text-primary">{m.nome}</span>
                    <div className="flex gap-1">
                      <button
                        type="button"
                        onClick={() => setPresencas((p) => ({ ...p, [m.usuario_id]: { ...estado, presente: true } }))}
                      ...
                      <button
                        type="button"
                        onClick={() => setPresencas((p) => ({ ...p, [m.usuario_id]: { ...estado, presente: false } }))}
                        className={`rounded-md px-3 py-1 text-xs font-semibold ${
                  ...
                  {!estado.presente && (
                    <input
                      ...
                      onChange={(e) =>
                        setPresencas((p) => ({ ...p, [m.usuario_id]: { ...estado, justificativa: e.target.value } }))
                      }
            ...
            {membros.length === 0 && <p className="text-sm text-text-secondary">Nenhum membro para registrar.</p>}
          </div>
        </div>
